...
```ts
import { createPiiEncryptionService, createPiiEncryptionOnlyService } from '@octabits-io/foundation/pii';
...
```ts
import { createBlindIndexService } from '@octabits-io/foundation/pii';
...
```ts
import { createEnvVarMasterKeyProvider } from '@octabits-io/foundation/pii';
...
about SQL, drivers, or ORMs (so `@octabits-io/foundation/pii` has **no `drizzle-orm`
peer**). The Postgres/Drizzle implementation of the seam ships separately as
`createDrizzleScopedKeyStore` in
`@octabits-io/foundation/drizzle/scoped-key-store` (column
...
```ts
import { createScopedKeyService } from '@octabits-io/foundation/pii';
import { createDrizzleScopedKeyStore } from '@octabits-io/foundation/drizzle/scoped-key-store';
...
```ts
import { encryptHybrid, decryptHybrid } from '@octabits-io/foundation/pii';
import { encryptSymmetric, decryptSymmetric, generateSymmetricKey } from '@octabits-io/foundation/pii';
...
### `@octabits-io/foundation/drizzle/*`
...
#### `@octabits-io/foundation/drizzle/db`
...
} from '@octabits-io/foundation/drizzle/db';
...
#### `@octabits-io/foundation/drizzle/factory`
...
```ts
import { Pool } from 'pg';
import { createDrizzle } from '@octabits-io/foundation/drizzle/factory';
...
#### `@octabits-io/foundation/drizzle/migrate`

Migration runner for Drizzle SQL migrations.

```ts
import { runMigrations } from '@octabits-io/foundation/drizzle/migrate';
...
| `encryptionKeyColumns`  | Per-scope PII encryption material (Age recipient + encrypted identity + blind-index key). Pairs with `@octabits-io/foundation/pii` — skip it if you don't use that package. |
...
import { baseScopeColumns, scopedConfigColumns } from "@octabits-io/foundation/drizzle/scope";
...
#### `@octabits-io/foundation/drizzle/crud`
...
#### `@octabits-io/foundation/drizzle/scoped-key-store`

The Drizzle adapter behind `@octabits-io/foundation/pii`'s structural
...
#### `@octabits-io/foundation/drizzle/config`
...
- Encryption is an injected `cipher` (raw-string `encrypt`/`decrypt`) — no
  `@octabits-io/foundation/pii` dependency; the engine owns the envelope + JSON. A
...
#### `@octabits-io/foundation/drizzle/rls`
...
#### `@octabits-io/foundation/drizzle/idempotency`
...
```ts
import { createICalFetcherService } from '@octabits-io/foundation/ical';
...
```ts
import { createICalParserService } from '@octabits-io/foundation/ical';
