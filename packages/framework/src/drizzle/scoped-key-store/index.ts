/**
 * @octabits-io/framework/drizzle/scoped-key-store — the Drizzle adapter behind
 * `@octabits-io/framework/pii`'s structural `ScopedKeyStore` seam.
 *
 * `@octabits-io/framework/pii` owns the encryption logic but knows nothing about SQL,
 ...
 * no import) — the same decoupling `@octabits-io/framework/drizzle/config` uses
 ...
 * don't hide the code. Moved here from `@octabits-io/framework/pii` — the SQLSTATE is an
