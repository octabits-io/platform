/**
 * @octabits-io/framework/drizzle/scope — generic Drizzle schema primitives for a
 ...
 * import { baseScopeColumns } from '@octabits-io/framework/drizzle/scope';
 *
 ...
 * import { encryptionKeyColumns } from '@octabits-io/framework/drizzle/scope';
 *
 ...
 * `@octabits-io/framework/pii`'s `createScopedKeyService` relies on that uniqueness to
 * resolve exactly one key row per scope, and the lost-race →
 * `scoped_key_store_conflict` signal in
 * [`@octabits-io/framework/drizzle/scoped-key-store`](./scoped-key-store) — the
 * Drizzle adapter behind pii's `ScopedKeyStore` seam — relies on the resulting
 * unique-violation (SQLSTATE 23505).
 *
 * **Pairs with `@octabits-io/framework/pii`**: the column shapes (Age recipient,
 * master-key-encrypted Age identity, blind-index HMAC key) are specific to that
 * package's encryption scheme. If your app does not use `@octabits-io/framework/pii`,
 ...
 * import { scopedConfigColumns } from '@octabits-io/framework/drizzle/scope';
 *
