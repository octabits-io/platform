...
| `./proposal` | The AI reviewable-outcome contract: a run's result as typed operations (`update`/`create`/`delete`/`reorder`) against existing records, with what each replaces, the principal that produced it (agent, on behalf of whom, under which grant), a reversibility class per operation, provenance, drift guard, partial accept, apply-side helpers (`detectDrift`, `invertOperations`), zod wire schemas. zod only, browser-safe, imports no other module | [proposal](./docs/proposal.md) |
...
| `./drizzle/*` | Drizzle ORM helpers: error→Result mapping, factory, migrations, one-shot data backfills, scope column-sets, CRUD, RLS (+ GUC-scope factory bridging `./ioc`), idempotency, scoped config, scoped-key store, job-audit store, agent ledger (who acted under whose grant, and how to undo it) | [foundation](./docs/foundation.md) |
| `./ical` | iCal ingestion: fetcher + RRULE-expanding parser + day-blocking layer | [foundation](./docs/foundation.md) |
| `./events` `./events/postgres` `./events/pglite` `./drizzle/event-outbox` `./hono/events` | Two-lane event fan-out: transactional outbox + `pg_notify` (durable, replayable) and inline notifications (ephemeral); LISTEN client (dedicated `pg` connection, or in-process on an embedded PGlite instance), relay, in-process hub, SSE endpoint as a plain fetch handler | [events](./docs/events.md) |
