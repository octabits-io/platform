...
| POST | `/api/ai/workflows/:id/apply` | Apply a reviewer's `ProposalDecision`: drift guard (409 `proposal_drift`), writes, a ledger row naming the agent and on whose behalf (`…/proposal` + `…/drizzle/agent-ledger` + `ai/proposals.ts`) |
| POST | `/api/ai/workflows/:id/revert` | Undo an application from its ledger row (`invertOperations`); irreversible operations are named, not undone |
...
| `./proposal` | [`ai/workflows.ts`](./src/ai/workflows.ts) (`propose` step — the producer: `proposeFields` with `guard: driftDigest(current)`, `proposeCreate`, `skipped[]`, `provenance`; the proposal is the step's output schema) + [`ai/proposals.ts`](./src/ai/proposals.ts) (the host's half: anchor→table mapping, `validateProposal` → `resolveDecision` → `detectDrift` → writes → ledger row; revert via `invertOperations` through the same writes). The reference implementation the contract's docs point at — `docs/proposal.md` "The recipe" | ✅ |
| `./drizzle/agent-ledger` | [`db/schema.ts`](./src/db/schema.ts) (`agentLedger` from `agentLedgerColumns`, unscoped) + [`ai/proposals.ts`](./src/ai/proposals.ts) — one row per applied AI action: the agent as principal (`ai:contact-brief`, `onBehalfOf` the applying role), mode `reviewed`, the decision, the operations as written, created ids, reversibility; `appliedAt` on the workflow wire view is `findByWorkflows` through the flow route factory's `load` seam; revert reads the row and marks it, never deletes it | ✅ |
| `./hono/flow` | [`routes/ai.ts`](./src/routes/ai.ts) — `createFlowWorkflowRoutes` serves the generic workflow read/control routes (list/active/get/snapshot/cancel/resume) over flow's public wire view, mounted with `app.route('/workflows', …)` (the Hono factory has no `prefix` option — where you mount it is the prefix); `appliedAt` rides the `extendWorkflow` seam — projected from the agent ledger (latest row per workflow), batched once per request through `load` — and `ai_quota_exceeded → 429` (plus the `proposal_*` conflicts) via `errorOverrides`. Only the domain trigger route and `/usage` are hand-written. | ✅ |
