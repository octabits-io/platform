...
| `components/ProposalReviewCard.vue` | [`AppProposalReviewCard.ts`](./app/components/AppProposalReviewCard.ts) re-export in `AiContactBrief.vue` — renders the run's stored `Proposal` (`output.propose`: what each value replaces, editable, partially acceptable) and emits a `ProposalDecision`, posted as-is to `POST /api/ai/workflows/:id/apply`; `appliedAt` comes back on the workflow from the server's audit row, and a **Revert** button posts to `…/revert`. The review loop end to end, with the server owning apply | ✅ |
| `components/AiResultReviewCard.vue` | [`AppAiResultReviewCard.ts`](./app/components/AppAiResultReviewCard.ts) re-export — kept registered for the field-list case; the contact brief moved to the proposal card above | ✅ |
